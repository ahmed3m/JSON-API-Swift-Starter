...
//        exerciseThree()
        ...
        Alamofire.request(.GET, apiToContact).validate().responseJSON() { response in
            ...
            case .Success:
                if let value = response.result.value {
                    ...
                    let randomIndex = Int(arc4random_uniform(UInt32(allMoviesData.count)))
                    self.movie = Movie(json: allMoviesData[randomIndex])
                    ...
                    self.loadPoster(self.movie!.posterLink)
                }
    ...
    @IBAction func viewOniTunesPressed(sender: AnyObject) {
        UIApplication.sharedApplication().openURL(NSURL(string: movie!.link)!)
    }
